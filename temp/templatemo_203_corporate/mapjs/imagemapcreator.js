...
function draw() {
	var codeTemplate = '<div id="imgmapdiv">\n\t<map name="imgmap">\n\t</map>\n</div>\n\n<p>\n\t<img src="peta_kabat.jpg" width="345" height="312" alt="Move mouse over image" usemap="#imgmap">\n</p>'; 
	...
	function createImagemap() {
		...
		if (x_arr.length > 2) {
			...
			polyHtml = '\t<area shape="poly" coords="'+poly+'" href="#" alt="'+number+++'">\n';
			
			data = data.replace(/<\/map>/i, polyHtml+'\t</map>');
			$('code').value = data;
			dp.SyntaxHighlighter.HighlightAll('code');
		}
	}
