...
class Adminkec extends Controller {
	...
	function Adminkec()
	{
		...
		$this->load->plugin();
		$this->load->model(array('Web_model','Adminkecamatan_model','Admin_model'));
		session_start();
	}
	function index()
	{
		...
		$identitas = $this->Adminkecamatan_model->Identitas_User($pecah[4]);
		...
		$kd_wil=$pecah[3];
			...
			{
				$this->load->view('admin_kec/bg_atas',$data);
				$this->load->view('admin_kec/isi_home',$data);
			}
		}
		else{
			?>
	...
	function managemenu()
	{
		...
		$kd_wil=$pecah[3];
			...
			else
			{
				...
				$data["menu_default"] = $this->Adminkecamatan_model->Semua_Menu_Induk($pecah[2]);
				$data["menu_induk_db"] = $this->Adminkecamatan_model->Menu_Induk_Database($pecah[2]);
				$this->load->view('admin_kec/bg_atas',$data);
				$this->load->view('admin_kec/manage_menu',$data);
			}
		}
		else{
			?>
	...
	function updatemenu()
	{
		...
		$kd_wil=$pecah[3];
			...
			else
			{
				...
				$this->Adminkecamatan_model->Update_Menu($query,$query_delete);
				echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/adminkec/managemenu'>";
			}
		}
		else{
			?>
	...
	function managekonten()
	{
		...
		$kd_wil=$pecah[3];
			...
			else
			{
				...
				$data["menu_induk_db"] = $this->Adminkecamatan_model->Menu_Induk_Database($pecah[2]);
					$data["konten"] = $this->Adminkecamatan_model->Konten_Per_Kat($pecah[2],$limit_konten,$ofset_konten);
					$tot_hal = $this->Adminkecamatan_model->Total_Konten_Per_Kat($pecah[2]);
					$config['per_page'] = $limit_konten;
					$config['base_url'] = base_url() . '/index.php/adminkec/managekonten';
				...
				$this->load->view('admin_kec/bg_atas',$data);
				$this->load->view('admin_kec/manage_konten',$data);
			}
		}
		else{
			?>
	...
	function tambahkonten()
	{
		...
		$kd_wil=$pecah[3];
			...
			else
			{
				...
				$data["menu_induk_db"] = $this->Adminkecamatan_model->Semua_Sitemap();
				$data["medialist"]=$this->Adminkecamatan_model->getDataMedia($pecah[2]);
				$this->load->view('admin_kec/bg_atas',$data);
				$this->load->view('admin_kec/tambah_konten',$data);
			}
		}
		else{
			?>
	...
	function simpankonten()
	{
		...
		$kd_wil=$pecah[3];
			...
			else
			{
				...
				$this->Adminkecamatan_model->Input_Konten('tbl_konten',$datainput);
				echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/adminkec/managekonten'>";
			}
		}
		else{
			?>
	...
	function media_list() 
	{
		$session=isset($_SESSION['username_belajar']) ? $_SESSION['username_belajar']:'';
		$pecah=explode("|",$session);
		$imglist=$this->Adminkecamatan_model->getDataMedia($pecah[2]);
	...
	function uploadfile()
	{
		...
		$this->Adminkecamatan_model->Simpan_Gambar($data);
		?>
	...
	function hapusmedia() 
	{
		...
		$imgdb=  $this->Adminkecamatan_model->getDataMedia($pecah[2]);
	   ...
	   $this->Adminkecamatan_model->Hapus_Media($id);
		?>
		<script type="text/javascript">
			javascript:history.go(-1);
		</script>
	...
	function editkonten()
	{
		...
		$kd_wil=$pecah[3];
			...
			else
			{
				...
				$data["menu_induk_db"] = $this->Adminkecamatan_model->Semua_Sitemap();
				$data["medialist"]=$this->Adminkecamatan_model->getDataMedia($pecah[2]);
				$data["edit_konten"]=$this->Adminkecamatan_model->Edit_Konten($id);
				$this->load->view('admin_kec/bg_atas',$data);
				$this->load->view('admin_kec/edit_konten',$data);
			}
		}
		else{
			?>
	...
	function updatekonten()
	{
		...
		$kd_wil=$pecah[3];
			...
			else
			{
				...
				$this->Adminkecamatan_model->Update_Konten('tbl_konten',$datainput,'kode_konten');
				?>
				<script type="text/javascript">
					javascript:history.go(-1);
				</script>
	...
	function hapuskonten()
	{
		...
		$kd_wil=$pecah[3];
			...
			{
				$this->Adminkecamatan_model->Hapus_Konten($id,'kode_konten','tbl_konten');
				?>
				<script type="text/javascript">
					javascript:history.go(-1);
				</script>
		...
		$kd_wil=$pecah[3];
			...
			else
			{
				...
				$data["menu_induk_db"] = $this->Adminkecamatan_model->Menu_Induk_Database($pecah[2]);
					...
					$data["paginator"] =$this->pagination->create_links();
				$this->load->view('admin_kec/bg_atas',$data);
				$this->load->view('admin_kec/manage_komentar',$data);
			}
		}
		else{
			?>
	...
	function kumkomentar()
	{
		...
		$kd_wil=$pecah[3];
			...
			{
				$data["konten"] = $this->Adminkecamatan_model->Tampil_Konten_Komentar('tbl_konten',$id);
				$data["komentar"] = $this->Adminkecamatan_model->Tampil_Konten_Komentar('tbl_komentar',$id);
				$this->load->view('admin_kec/bg_atas',$data);
				$this->load->view('admin_kec/kumpulan_komentar',$data);
			}
		}
		else{
			?>
	...
	function editkomentar()
	{
		...
		$kd_wil=$pecah[3];
			...
			{
	   			$data['scriptmce'] = $this->scripttiny_mce($pecah[2]);
				$data["komen"] = $this->Adminkecamatan_model->Tampil_Detail_Komentar('tbl_komentar',$id);
				$this->load->view('admin_kec/bg_atas',$data);
				$this->load->view('admin_kec/edit_komentar',$data);
			}
		}
		else{
			?>
	...
	function updatekomentar()
	{
		...
		$kd_wil=$pecah[3];
			...
			else
			{
				...
				$this->Adminkecamatan_model->Update_Konten('tbl_komentar',$datainput,'kode_komentar');
				?>
				<script type="text/javascript">
	...
	function hapuskomentar()
	{
		...
		$kd_wil=$pecah[3];
			...
			{
				$this->Adminkecamatan_model->Hapus_Konten($id,'kode_komentar','tbl_komentar');
				?>
				<script type="text/javascript">
					javascript:history.go(-1);
				</script>
	...
	function manageakun()
	{
		...
		$kd_wil=$pecah[3];
			...
			{
				$data["komen"] = $this->Adminkecamatan_model->Edit_User($pecah[4]);
				$this->load->view('admin_kec/bg_atas',$data);
				$this->load->view('admin_kec/manage_akun',$data);
			}
		}
		else{
			?>
			...
			else
			{
				...
				if($passlama!="" && $datainput['password']!="")
				{
					...
					else if($passlama!="")
					{
						...
						$this->Adminkecamatan_model->Update_Akun_Password($datainput);
						?>
						<script type="text/javascript">
				...
				else
				{
					...
					$datainput['username'] = $this->input->post('username');
					$datainput['kode_user'] = $this->input->post('kd_user');
					$this->Adminkecamatan_model->Update_Akun($datainput);
					?>
					<script type="text/javascript">
						javascript:history.go(-1);
					</script>
...
<script type="text/javascript">
	tinyMCE.init({
		...
		external_image_list_url : "'.base_url().'index.php/adminkec/media_list/'.$selectcategory.'",
