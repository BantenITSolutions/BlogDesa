...
foreach($konten->result_array() as $k)
{
	...
	$hit_komen = $this->Superadmin_model->Hitung_Komentar($k['kode_konten']);
	foreach($hit_komen->result() as $hk)
	{
		echo "<td>".$hk->jum."</td><td><a href='".base_url()."index.php/superadmin/kumkomentar/".$k['kode_konten']."'>Lihat Komentar</a></td></tr>";
	}
	$i++;
}
?>
